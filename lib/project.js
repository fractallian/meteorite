...
Project = function(root, meteorArgs) {
  ...
  this.root = root;
  this.overrideRoot = meteorArgs.overrideRoot; // packages defined in a smart.json here will override those in root
  this.meteorArgs = meteorArgs;
  this.smartJsonPath = path.join(this.root, 'smart.json');
  this.smartLockPath = path.join(this.overrideRoot || this.root, 'smart.lock');
...
Project.prototype.checkSmartJson = function(forceUpdate) {
  ...
  // this is the config specified in smart.json (and its override if applicable)
  var config = new Config(this.root, this.overrideRoot);

...
Config = function(root, overrideRoot) {
  this._parseRoot(root);
  if (overrideRoot)
    this._parseRoot(overrideRoot, true);
};

Config.prototype._parseRoot = function(root, isOverride) {
  // Build up path
  ...
  if (fs.existsSync(configPath)) {
    ...
    // This isn't the first root being parsed, we only care about packages.
    if (isOverride) {
      _.extend(this.packages, config.packages);
    } else {
